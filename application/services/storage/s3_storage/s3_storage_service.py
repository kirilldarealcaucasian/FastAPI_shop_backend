...
class S3StorageService(EntityBaseService):
    ...
    def __init__(
        ...
        super().__init__(image_repo=image_repo, book_repo=book_repo)
    ...
    async def delete_instance_with_images(
        ...
        if delete_images:
            ...
            _ = await super().delete(
                repo=self.book_repo, session=session, instance_id=instance_id
            ...
            try:
                ...
                logger.error(
                    "Failed to delete objects from remote bucket", exc_info=True
                )
                raise RemoteBucketDeletionError()
            ...
            try:
                await self.book_repo.delete(session=session, instance_id=instance_id)
                await super().commit(session=session)
            except Exception:
                await session.rollback()
                logger.error(
