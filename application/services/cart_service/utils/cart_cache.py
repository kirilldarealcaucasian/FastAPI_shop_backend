...
def get_cart_from_cache(func: Callable):
    ...
    async def wrapper(
        ...
        **kwargs,
        ...
        cart_set_name = (
            f"cart:{shopping_session_id}"  # name of a set where book_ids are stored
        )
        ...
        return ReturnCartS(cart_id=shopping_session_id, books=deserialized_cart_books)
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            for book in assembled_cart.books:
                await serialize_and_store_cart_books(book=book, redis_con=redis_con)
            ...
            for book_id in books_ids:
                ...
                await redis_con.sadd(cart_set_name, book_id)
                await redis_con.expire(
                    name=f"cart:{assembled_cart.cart_id}", time=cache_time_seconds
