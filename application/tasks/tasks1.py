...
def create_image_folder(concrete_image_folder_name: str) -> str:
    ...
    try:
        ...
        logger.error(
            "Os Exception: Unable to create image directory", extra, exc_info=True
        )
...
def fix_and_save_image(image_io: io.BytesIO, full_image_path: str):
    try:
        ...
        logger.error(
            "Os Error: Error while opening or saving image", extra, exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
def send_order_summary_email(
    ...
    email: EmailMessage = email_generator.create_order_confirmation_template(
        data=order_data
    )
