...
from .associations import BookAuthorsAssoc, BookCategoryAssoc
...
class Book(Base, TimestampMixin):
    ...
    language: Mapped[str]
    country: Mapped[str]
    price_per_unit: Mapped[Decimal] = mapped_column(DECIMAL, server_default=text("0"))
    ...
    categories: Mapped[list["Category"]] = relationship(
        back_populates="books", secondary=BookCategoryAssoc
    )
    authors: Mapped[list["Author"]] = relationship(
        back_populates="books", secondary=BookAuthorsAssoc
    )
    ...
    __table_args__ = (
        CheckConstraint(
            "number_in_stock >= 0", name="ck_books_number_in_stock_positive"
        ),
    )
